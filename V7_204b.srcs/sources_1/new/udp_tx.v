...
localparam  udp_tx_idle      = 7'b000_0000; //初始状态，等待开始发送信号
localparam  udp_tx_check_sum = 7'b000_0001; //IP首部校验和
localparam  udp_tx_preamble  = 7'b000_0010; //发送前导码+帧起始界定符
localparam  udp_tx_eth_head  = 7'b000_0100; //发送以太网帧头
localparam  udp_tx_ip_head   = 7'b000_1000; //发送IP首部+UDP首部
localparam  udp_tx_tx_data   = 7'b001_0000; //发送数据
localparam  udp_tx_tx_data_1 = 7'b010_0000;
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        if(pos_start_en && cur_state==udp_tx_idle) begin
            ...
            //IP长度：有效数据+IP首部长度            
            total_num <= tx_byte_num + 16'd28 +16'd32;  
            //UDP长度：有效数据+UDP首部长度            
            udp_num <= tx_byte_num + 16'd8+16'd32;               
        end    
    end
end
...
always @(*) begin
    ...
    case(cur_state)
        ...
        udp_tx_ip_head : begin                                //发送IP首部+UDP首部               
            if(skip_en)
                next_state = udp_tx_tx_data_1;
            else
                next_state = udp_tx_ip_head;      
        end
        udp_tx_tx_data_1 : begin                                //发送数据                  
            if(skip_en)
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        case(next_state)
            ...
            udp_tx_tx_data_1:begin
                crc_en <= 1'b1;
                gmii_tx_en <= 1'b1;
                gmii_txd <= header_data[cnt*8 +:8];
                if(cnt == 5'd31) begin
                    skip_en <= 1'b1;   
                    cnt <= 5'd0;   
